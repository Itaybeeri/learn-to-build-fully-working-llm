...
Short, plain-language definitions for every term the course uses; the full explanation lives in
each term's concept note. **Grouped by the module that introduces each term** (same organization
as the index, the notes, and the portal). A term is listed once, under the module where it's
first taught — so the section headings also tell you *when* you'll meet each word.
